...
const submitApplicationTool: FunctionDeclaration = {
  ...
  description: 'Submit the user\'s application for assistance after gathering necessary details.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      need_type: {
      ...
      details: {
        type: Type.STRING,
        description: 'Summary of the user\'s situation and why they need help.',
      }
    },
    required: ['need_type', 'details']
  }
};
...
export const useGeminiLive = () => {
  ...
  const { submitIntakeRequest } = useStore();
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: {
             parts: [{ text: "You are Windy, a helpful intake assistant for SecondWind. Your job is to have a quick, friendly voice chat to qualify a user for aid (Rent, Laptop, Bus Pass). Be concise, warm, and professional. Once you have the info (Need, Urgency, Situation), call the 'submit_application' tool." }]
          },
        ...
        callbacks: {
          onopen: () => {
            ...
            processor.onaudioprocess = (e) => {
               ...
               setVolume(Math.min(1, rms * 5)); // Boost input gain visually
             ...
             if (msg.toolCall) {
                for (const fc of msg.toolCall.functionCalls) {
                   if (fc.name === 'submit_application') {
                      submitIntakeRequest({
                         type: (fc.args as any).need_type,
                         details: (fc.args as any).details
                      });
                      // Respond to model
                      sessionPromise.then(session => session.sendToolResponse({
                        functionResponses: {
                           name: fc.name,
                           id: fc.id,
                           response: { result: 'Application Submitted Successfully' }
                        }
                      }));
                   }
                }
  ...
  }, [isConnected, submitIntakeRequest]);
