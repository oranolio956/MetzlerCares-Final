...
const WorldMap: React.FC<{ activeTx: string | null }> = ({ activeTx }) => {
  ...
  return <div ref={containerRef} className="w-full h-full"><canvas ref={canvasRef} className="w-full h-full object-contain opacity-60 mix-blend-multiply" /></div>;
};
    ...
    <div className="w-full max-w-6xl mx-auto px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12">
         <div>
            ...
            <h2 className="font-display font-bold text-3xl md:text-5xl text-brand-navy leading-none">Open Books.</h2>
            <p className="text-brand-navy/60 text-base md:text-lg mt-4 max-w-lg">Radical transparency. Every cent is tracked from donation to vendor payment in real-time.</p>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-3 bg-brand-cream border-2 border-brand-navy/5 rounded-[2.5rem] h-48 md:h-96 relative overflow-hidden flex items-center justify-center shadow-inner">
             ...
             {activeTx && (
                <div className="absolute bottom-6 right-6 left-6 md:left-auto bg-white/90 backdrop-blur p-4 rounded-xl shadow-lg border-l-4 border-brand-teal animate-slide-up max-w-sm">
                   ...
                   <p className="text-sm font-bold text-brand-navy">Payment of ${MOCK_LEDGER.find(t => t.id === activeTx)?.amount} routed to {MOCK_LEDGER.find(t => t.id === activeTx)?.vendor}.</p>
                </div>
             )}
          </div>
          ...
          <div className="lg:col-span-3 bg-white rounded-[2.5rem] shadow-xl border border-brand-navy/5 overflow-hidden">
            ...
            <div className="border-b border-brand-navy/5 p-4 md:p-6 flex items-center justify-between bg-brand-cream/30">
                <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-xs font-bold uppercase tracking-widest text-brand-navy/40">Live Feed</span>
                </div>
                <button className="text-brand-navy/40 hover:text-brand-navy transition-colors"><Filter size={20} /></button>
            </div>
            ...
            <div className="divide-y divide-brand-navy/5">
                ...
                <div key={item.id} className={`grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 px-6 md:px-8 py-4 md:py-6 items-start md:items-center hover:bg-brand-cream/50 transition-colors group cursor-default ${activeTx === item.id ? 'bg-brand-teal/5' : ''}`} onMouseEnter={() => setActiveTx(item.id)} onMouseLeave={() => setActiveTx(null)}>
                    ...
                    <div className="col-span-3 mt-2 md:mt-0 w-full">
                         ...
                         <div className="flex items-center gap-2 md:gap-3">
                            <div className="hidden md:flex w-8 h-8 rounded-full bg-brand-navy/5 items-center justify-center text-brand-navy group-hover:bg-brand-navy group-hover:text-white transition-colors shrink-0"><ArrowDownLeft size={14} /></div>
                            <div className="text-left"><span className="block font-bold text-brand-navy text-sm">{item.vendor}</span><span className="font-mono text-[10px] text-brand-navy/30 hidden md:block">{item.id}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="col-span-3 flex justify-between md:justify-start items-center mt-2 md:mt-0 w-full">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${item.category === 'RENT' ? 'bg-brand-coral/10 text-brand-coral' : item.category === 'TECH' ? 'bg-brand-teal/10 text-brand-teal' : 'bg-brand-lavender/20 text-brand-navy'}`}>{item.category}</span>
                    </div>
            ...
            <div className="p-4 md:p-6 bg-brand-cream/30 border-t border-brand-navy/5 text-center">
                <span className="text-[10px] md:text-xs font-bold text-brand-navy/30 uppercase tracking-widest">End of Live Record • Showing last 6 transactions</span>
            </div>
          </div>
      </div>
    </div>
