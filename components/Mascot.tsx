...
export interface MascotProps {
  ...
  thought?: string | null; // New Prop: Contextual thought bubble
}
...
export const Mascot: React.FC<MascotProps> = memo(({ 
  ...
  useEffect(() => {
    ...
    const resetIdle = () => {
        ...
        if (['happy', 'default', 'thinking'].includes(expression)) {
            idleTimer = setTimeout(() => setIsAsleep(true), 10000); 
        }
    };
  ...
  return (
    <div className={`relative ${className}`}>
        {/* Thought Bubble - Positioned relative to container */}
        {thought && !isAsleep && (
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-48 z-20 animate-in slide-in-from-bottom-4 fade-in duration-500">
                <div className="bg-white/90 backdrop-blur-sm border border-brand-navy/10 p-3 rounded-2xl rounded-bl-none shadow-lg text-xs font-bold text-brand-navy text-center leading-tight">
                    {thought}
                </div>
                <div className="w-3 h-3 bg-white/90 border-l border-b border-brand-navy/10 transform -rotate-45 absolute -bottom-1.5 left-4"></div>
            </div>
        )}
