...
const MessageBubble: React.FC<{ message: Message; isLast: boolean }> = ({ message, isLast }) => {
  ...
  return (
    <div
      className={`max-w-[85%] md:max-w-[80%] p-3 md:p-4 rounded-2xl text-base md:text-lg font-medium leading-relaxed shadow-sm transition-all duration-300 ${
        message.role === 'user'
...
export const IntakeChat: React.FC = () => {
  ...
  if (!hasStarted) {
    return (
      <div className="h-[80vh] md:h-[600px] w-full max-w-2xl mx-auto bg-white rounded-3xl shadow-[8px_8px_0px_0px_rgba(26,42,58,1)] border-4 border-brand-navy overflow-hidden relative flex flex-col items-center justify-center p-6 md:p-8 text-center">
         ...
         <h2 className="font-display font-bold text-2xl md:text-3xl text-brand-navy mb-3">Hi, I'm Windy.</h2>
         <p className="text-brand-navy/60 max-w-sm mb-8 leading-relaxed text-sm md:text-base">
           I'm here to help you get the resources you need. No judgment, just a quick chat to see how we can help.
         </p>

         <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 w-full max-w-md mb-8">
         ...
         <button 
           ...
           className="bg-brand-navy text-white text-base md:text-lg font-bold px-8 py-4 rounded-xl hover:bg-brand-teal hover:scale-105 transition-all shadow-lg flex items-center gap-3 w-full sm:w-auto justify-center"
         >
  ...
  return (
    <div className="flex flex-col h-[80vh] md:h-[600px] w-full max-w-2xl mx-auto bg-white rounded-3xl shadow-2xl border-4 border-brand-navy overflow-hidden relative">
      {/* Header */}
      <div className={`p-4 flex items-center gap-4 text-white z-10 transition-colors duration-500 ${mode === 'voice' ? 'bg-brand-teal' : 'bg-brand-navy'}`}>
        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-white transition-all duration-300 ${mascotExpression === 'thinking' ? 'bg-brand-lavender scale-110 shadow-[0_0_15px_rgba(167,172,217,0.5)]' : mascotExpression === 'excited' ? 'bg-brand-coral scale-110' : 'bg-brand-teal'}`}>
           <Mascot expression={mascotExpression} />
        </div>
        <div>
          <h3 className="font-display font-bold text-lg md:text-xl">Windy</h3>
      ...
      <div className="flex-1 overflow-hidden relative bg-brand-cream flex flex-col">
        ...
        {mode === 'voice' && (
           <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-brand-cream p-4">
              <div 
                 className="relative transition-all duration-75"
                 style={{ transform: `scale(${1 + volume})` }}
              >
                  <div className="w-48 h-48 md:w-64 md:h-64 opacity-10 bg-brand-teal rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 blur-3xl animate-pulse"></div>
                  <Mascot expression={mascotExpression} className="w-40 h-40 md:w-48 md:h-48" />
              </div>
              
              <div className="mt-8 md:mt-12 text-center space-y-2">
                 {isConnected ? (
                    <p className="text-brand-navy font-bold text-lg md:text-xl animate-pulse">
        ...
        {/* TEXT MODE MESSAGES */}
        <div className={`flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 relative transition-opacity duration-300 ${mode === 'voice' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      {mode === 'text' && (
        <div className="p-3 md:p-4 bg-white border-t-2 border-brand-navy/10 z-30">
          <div className={`flex items-center gap-2 bg-brand-cream rounded-xl border-2 p-1.5 md:p-2 transition-all duration-300 ${isAiTyping ? 'opacity-50 pointer-events-none border-brand-navy/10' : 'border-brand-navy focus-within:ring-4 focus-within:ring-brand-teal/20'}`}>
            <input
              type="text"
              className="flex-1 bg-transparent px-3 md:px-4 py-2 outline-none text-brand-navy font-bold placeholder-brand-navy/40 text-base"
            ...
            <button
              ...
              className={`p-2 md:p-3 rounded-lg transition-all duration-300 border-2 border-brand-navy shadow-[2px_2px_0px_0px_rgba(26,42,58,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] ${
