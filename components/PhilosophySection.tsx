...
export const PhilosophySection: React.FC<PhilosophySectionProps> = ({ onNavigate, isCalmMode }) => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.5 } // Trigger when 50% of text is visible
    );
  ...
  // Determine styles for accessibility and motion
  const isBrokenHidden = viewMode === 'fixed';
  const isFixedHidden = viewMode === 'broken';

  return (
    // Atmospheric Background Shift: Changes color based on viewMode
        ...
        <div className="flex flex-col items-center w-full max-w-7xl mx-auto px-4 md:px-0">
          ...
          <div className="text-center mb-12 relative z-10 w-full">
            <div className="inline-block relative">
              <h2 
                ref={textRef} 
                className="font-display text-4xl md:text-7xl font-bold text-brand-navy leading-tight opacity-30 select-none blur-[1px]"
              >
                Charity is broken.
              </h2>
              ...
              <svg className="absolute top-1/2 left-[-5%] w-[110%] h-[40px] overflow-visible pointer-events-none" style={{ transform: 'translateY(-50%)' }} aria-hidden="true">
                  <path 
                    ...
                    strokeWidth="6" 
                    strokeLinecap="round"
                    className={(!isCalmMode && isVisible) ? "animate-draw" : "opacity-0"} 
                    style={{ 
                      strokeDasharray: 1000, 
                      strokeDashoffset: 1000, 
                      animation: (!isCalmMode && isVisible) ? 'draw 1.5s ease-out forwards 0.2s' : 'none' 
                    }} 
                  />
              </svg>
            </div>
          ...
          >
            ...
            <div 
              ...
              style={{ 
                left: viewMode === 'broken' ? '0.5rem' : '50%', 
                width: 'calc(50% - 0.5rem)',
                backgroundColor: viewMode === 'broken' ? '#FF8A75' : '#2D9C8E'
              }}
            />
            
            <button 
              role="radio"
              aria-checked={viewMode === 'broken'}
              onClick={() => setViewMode('broken')}
              className={`flex-1 py-3 rounded-full font-bold text-sm md:text-base transition-colors duration-300 relative z-10 flex items-center justify-center gap-2 ${viewMode === 'broken' ? 'text-brand-navy' : 'text-brand-navy/50 hover:text-brand-navy/80'}`}
            >
              <AlertTriangle size={18} className={viewMode === 'broken' ? "text-brand-navy" : "text-brand-coral"} />
              The Old Way
            </button>
            <button 
              role="radio"
          ...
          >
            ...
            >
              ...
              {/* --- SIDE A: THE BROKEN WAY (Black Box) --- */}
              <article 
                ...
                `}
                style={{
                    transform: isCalmMode ? 'none' : (viewMode === 'broken' ? 'rotateX(0deg)' : 'rotateX(180deg)')
                }}
              >
                {/* Atmosphere: Fog of Bureaucracy */}
                ...
                {/* Floating "Red Tape" Particles */}
                <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
                    {[
                      { text: 'Admin Fees', top: '10%', left: '10%', delay: '0s' },
                      { text: 'Red Tape', top: '20%', left: '80%', delay: '1s' },
                      { text: 'Overhead?', top: '70%', left: '15%', delay: '2s' },
                      { text: 'Waiting List', top: '80%', left: '70%', delay: '0.5s' },
                      { text: 'Fax Machines', top: '15%', left: '50%', delay: '1.5s' }
                    ].map((item, i) => (
                      <div 
                        key={i}
                        className="absolute bg-white/50 px-3 py-1 rounded border border-brand-navy/5 text-brand-navy/40 font-bold text-xs md:text-sm uppercase tracking-widest animate-float backdrop-blur-sm"
                        style={{ 
                          top: item.top, 
                          left: item.left,
                      ...
                      >
                        <XCircle size={12} className="inline mr-1 mb-0.5 text-brand-coral" />
                        {item.text}
                      </div>
                    ))}
                </div>
                ...
                <div className="relative z-10 flex flex-col items-center justify-center h-full text-center p-6 md:p-12">
                    ...
                    <div className="w-24 h-24 md:w-40 md:h-40 bg-brand-navy rounded-3xl flex items-center justify-center shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] mb-8 relative transform -rotate-3 transition-transform hover:rotate-0 duration-500 group-hover:scale-105">
                      ...
                      {/* Mascot Reaction */}
                      <div className="absolute -right-8 -bottom-4 md:-right-12 md:-bottom-6 w-24 h-24 md:w-32 md:h-32 animate-wiggle origin-bottom" style={{ animationDuration: '4s' }}>
                    ...
                    <div className="flex flex-wrap justify-center gap-3 md:gap-4">
                      {['Unknown Impact', 'Slow Speed', 'High Overhead'].map((text, i) => (
                        <div key={i} className="bg-white/80 px-3 py-1.5 md:px-4 md:py-2 rounded-lg shadow-sm border border-brand-navy/5 text-brand-navy/60 font-bold text-xs md:text-sm flex items-center gap-2">
                            <Ban size={14} className="text-brand-coral" />
                            {text}
                        </div>
                      ))}
                    </div>
              ...
              {/* --- SIDE B: THE SECONDWIND WAY (Glass Box) --- */}
              <article 
                ...
                `}
                style={{
                    transform: isCalmMode ? 'none' : (viewMode === 'fixed' ? 'rotateX(0deg)' : 'rotateX(-180deg)')
                }}
              >
                ...
                <div className="relative z-10 h-full flex flex-col md:flex-row p-6 md:p-12 gap-8 md:gap-12 items-center">
                    ...
                    <div className="flex-1 text-center md:text-left pt-8 md:pt-0">
                      <div className="inline-flex items-center gap-2 bg-brand-teal/10 text-brand-teal px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-6 border border-brand-teal/20">
                          ...
                          Direct-to-Vendor Model
                      </div>
                      
                      <h3 className="text-3xl md:text-5xl font-display font-bold text-brand-navy mb-6 leading-tight">
                          We don't give cash. <br className="hidden md:block"/>
                          <span className="text-brand-teal bg-brand-teal/5 px-2 rounded-lg">We pay bills.</span>
                      </h3>
                      
                      <p className="text-brand-navy/70 text-base md:text-lg mb-8 leading-relaxed max-w-md mx-auto md:mx-0">
                          Donations fund specific invoices (Rent, Laptops, Bus Passes). 
                          Money goes directly to the Landlord, Best Buy, or the Transit Authority.
                          <span className="block mt-4 font-bold text-brand-navy flex items-center justify-center md:justify-start gap-2">
                            <CheckCircle2 size={18} className="text-brand-teal" />
                            Zero fraud. 100% impact.
                          </span>
                      </p>
                      
                      <button 
                        onClick={() => onNavigate('donate')}
                        className="bg-brand-navy text-white px-8 py-4 rounded-xl font-bold hover:bg-brand-teal hover:scale-105 transition-all shadow-[0_10px_20px_-5px_rgba(26,42,58,0.3)] flex items-center justify-center md:justify-start gap-3 w-full md:w-auto group relative overflow-hidden"
                      >
                    ...
                    <div className="flex-1 w-full h-[300px] md:h-full relative flex items-center justify-center perspective-500" aria-hidden="true">
                      ...
                      <div className="absolute -top-6 -right-6 md:-top-12 md:-right-0 z-50 animate-bounce" style={{ animationDuration: '3s' }}>
                          <div className="relative">
                            <Mascot expression="celebration" className="w-24 h-24 md:w-32 md:h-32 drop-shadow-xl" />
                            <div className="absolute -bottom-2 right-0 bg-white px-3 py-1 rounded-full text-[10px] font-bold text-brand-navy border border-brand-navy/10 shadow-sm whitespace-nowrap flex items-center gap-1">
                              <CheckCircle2 size={10} className="text-brand-teal" /> Verified!
                            </div>
                          </div>
                      </div>

                      {/* Card 1: Fraud Proof (Bottom) */}
                      <div className="absolute w-56 md:w-72 bg-white p-5 rounded-2xl border border-brand-navy/10 shadow-lg transform transition-all duration-500 hover:scale-105 hover:z-40 z-30 -rotate-6 -translate-x-2 md:-translate-x-8 hover:rotate-0 top-10 md:top-auto">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="p-2 bg-brand-coral/10 rounded-lg text-brand-coral">
                                <Lock size={18} />
                            </div>
                            <h4 className="font-bold text-brand-navy text-sm">Fraud Proof</h4>
                          </div>
                          <p className="text-[11px] text-brand-navy/60 font-medium leading-tight">Funds never touch personal bank accounts. Only verified business vendors are paid.</p>
                      </div>

                      {/* Card 2: Open Ledger (Middle) */}
                      <div className="absolute w-56 md:w-72 bg-white p-5 rounded-2xl border border-brand-navy/10 shadow-lg transform transition-all duration-500 hover:scale-105 hover:z-40 z-20 rotate-3 translate-x-1 md:translate-x-4 translate-y-8 md:translate-y-4 hover:rotate-0 top-16 md:top-auto">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="p-2 bg-brand-teal/10 rounded-lg text-brand-teal">
                                <Search size={18} />
                            </div>
                            <h4 className="font-bold text-brand-navy text-sm">Open Ledger</h4>
                          </div>
                          <p className="text-[11px] text-brand-navy/60 font-medium leading-tight">Every transaction is published to our public database in real-time. Anyone can audit.</p>
                      </div>

                      {/* Card 3: Receipts (Top) */}
                      <div className="absolute w-56 md:w-72 bg-white p-5 rounded-2xl border border-brand-navy/10 shadow-lg transform transition-all duration-500 hover:scale-105 hover:z-40 z-10 rotate-12 translate-x-4 md:translate-x-12 translate-y-16 md:translate-y-12 hover:rotate-0 top-24 md:top-auto hidden sm:block">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="p-2 bg-brand-yellow/20 rounded-lg text-brand-navy">
                                <FileText size={18} />
                            </div>
                            <h4 className="font-bold text-brand-navy text-sm">Receipts</h4>
                          </div>
                          <p className="text-[11px] text-brand-navy/60 font-medium leading-tight">Donors receive a digital copy of the actual receipt from the vendor.</p>
                      </div>
          ...
          <div className="mt-16 flex flex-col md:flex-row items-center gap-6 opacity-40 hover:opacity-100 transition-opacity duration-500">
            ...
            <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 filter grayscale mix-blend-multiply items-center">
                ...
                <span className="font-display font-bold text-brand-navy text-lg md:text-xl flex items-center gap-2">
                   <Fingerprint size={20} className="text-brand-coral" />
                   Verified Identity
                </span>
                <span className="font-display font-bold text-brand-navy text-lg md:text-xl flex items-center gap-2">
